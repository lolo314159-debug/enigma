...
st.set_page_config(page_title="Enigma : Version Optimisée", layout="wide")
alphabet = string.ascii_uppercase

# --- 2. LOGIQUE DE SIGNAL (TRAJET DIRECT) ---
def get_enigma_path(key_char, o1, o2, o3):
    idx0 = alphabet.index(key_char)
    # Le signal traverse les rotors en tenant compte des offsets
...
# --- 3. INTERFACE ---
st.title("📟 Simulateur Enigma : Visuel Optimisé")

# Le timer est bien présent
delay = st.sidebar.slider("Vitesse d'observation (sec)", 0.0, 5.0, 1.5, step=0.5)
...
with col_log:
    ...
    st.info(f"**Texte saisi :** `{st.session_state.text_in}`")
    st.success(f"**Résultat chiffré :** `{st.session_state.text_out}`")
...
with col_kbd:
    # Clavier AZERTY complet (26 touches)
...
# --- 4. VISUALISATION (LETTRES GROSSIES & ESPACE RÉDUIT) ---
def draw_viz():
    fig = go.Figure()
    # Niveaux resserrés pour un look plus mécanique
    levels = [1.6, 1.25, 0.9, 0.55] 
    ...
    # Trajet du signal (Lignes rouges épaisses)
    if path:
        for s in range(3):
    ...
    # Affichage des rotors avec lettres agrandies
    for l_idx, y_val in enumerate(levels):
        ...
        for i in range(26):
            ...
            fig.add_trace(go.Scatter(
                ...
                # Carrés plus grands
                marker=dict(symbol='square', size=25, color='white', 
                            line=dict(color="red" if is_active else "#eee", width=2 if is_active else 1)),
                text=char_to_show,
                # Police plus grosse et grasse
                textfont=dict(size=13, color="red" if is_active else "black", family="Arial Black"),
                showlegend=False
            ))
    ...
    fig.update_layout(height=450, margin=dict(l=10, r=10, t=10, b=10), plot_bgcolor='white',
                      ...
                      yaxis=dict(showgrid=False, range=[0.4, 1.8], showticklabels=False))
    return fig
...
1.  **Ligne du haut** : Représente le clavier (fixe).
2.  **Rotation (R1)** : À chaque touche, la première ligne de lettres se décale. Cela change physiquement le contact électrique utilisé pour le chiffrage.
3.  **Continuité** : Le signal suit une trajectoire électrique. S'il sort d'un rotor sur une position, il entre dans le suivant par cette même position exacte, garantissant un flux continu.
""")
