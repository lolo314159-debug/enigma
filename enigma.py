...
st.set_page_config(page_title="Enigma : Correction Chemin", layout="wide")
...
st.title("📟 Enigma : Signal et Rotation")
...
with col_log:
    ...
    c1, c2 = st.columns(2)
    if c1.button("⏪ Reset 'A'", use_container_width=True):
        st.session_state.off1, st.session_state.off2, st.session_state.off3 = 0, 0, 0
...
with col_kbd:
    ...
    for row in rows:
        ...
        for i, key in enumerate(row):
            # IMPORTANT : Pas de rerun() ici pour laisser le graphique s'afficher
    ...
    for stage in range(3):
        ...
        for i in range(26):
            ...
            fig.add_trace(go.Scatter(
                ...
                mode='lines', line=dict(color="red" if is_active else "lightgray", width=4 if is_active else 1),
...
# On affiche d'abord le graphique
container_graph = st.empty()
container_graph.plotly_chart(plot_enigma(), use_container_width=True)

# --- GESTION DU DÉLAI ET ROTATION (Après l'affichage) ---
if st.session_state.pressed_key:
    if delay > 0:
        time.sleep(delay)
